...
static func create_random_Notes_array(array_size: int, correct_answer_index: int, correct_answer: Note) -> Array[Note]:
	var notes_names_size = Note.notes_names[Note.naming_systems.ANGLO].size()
	var note_array : Array[Note] = [Note.new(1),Note.new(1),Note.new(1),Note.new(1)]
	note_array[correct_answer_index] = correct_answer
#	for ii in array_size:
